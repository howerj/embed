...
\ @todo modify VM so stack variables and registers are stored in header
...
* The current virtual machine implementation and metacompiler do not store
the program counter, the return and stack pointer, and the top of the stack
in image that is generated by the metacompiler and loaded/saved by the virtual
machine. These variables would be useful to have in the saved image. This would
allow all mention of the return and stack pointer being at specific addresses
to be removed, they would only be stored at specific locations in the header.
